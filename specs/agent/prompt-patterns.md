...
| Create both endpoints before creating a relationship | System prompt + tool enforcement | **Critical for preventing orphaned edges.** The manage_relationship tool rejects relationships where either endpoint doesn't exist. The prompt must instruct: "Before creating any relationship, ensure both the source and target entities exist. If the target entity is not in your assigned files, create a minimal stub entity with the properties you can infer from the reference (e.g., module path, name, URL). The stub will be enriched when the target's source files are processed by another worker." |
